...
def create_mock_profile():
	...
	for vendor in vendor_db:
		...
		transaction_history["spend"][vendor] = [
			{"amount": amount, "date": date} for amount,date in zip(rand_spend_amounts, rand_dates)
		]
	...
	for deposit in deposit_db:
		...
		transaction_history["deposit"][deposit] = [
			{"amount":amount, "date":date} for amount,date in zip(rand_deposit_amounts, rand_dates)
		]
